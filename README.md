...
<script type="text/babel">
    ...
    const AIChat = () => {
        ...
        return (
            <div className="flex flex-col h-full pt-16 pb-20 animate-up">
                ...
                <div className="fixed bottom-[60px] left-0 w-full bg-slate-900 border-t border-slate-800 p-3 z-40 max-w-md mx-auto">
                    <form onSubmit={handleSend} className="flex gap-2">
                        <input
                            type="text"
                            value={input}
                            onChange={(e) => setInput(e.target.value)}
                            placeholder="Ask Coach Bolt..."
                            className="flex-1 bg-slate-800 text-white rounded-full px-5 py-3 focus:outline-none focus:ring-1 focus:ring-lime-500 text-sm border border-slate-700"
                        />
                        <button type="submit" className="w-12 h-12 rounded-full bg-lime-500 text-slate-900 flex items-center justify-center shadow-lg shadow-lime-500/20 active:scale-95 transition-transform">
        ...
        return (
            <div className="pt-20 pb-24 px-4 min-h-screen animate-up">
                ...
                <div className="space-y-4">
                    {LESSONS.map(lesson => (
                        <div key={lesson.id} className="bg-slate-800 rounded-2xl border border-slate-700 overflow-hidden">
                            <div onClick={() => setExpandedId(expandedId === lesson.id ? null : lesson.id)} className="p-5 flex items-center justify-between cursor-pointer">
                                <div className="flex-1">
                                    <div className="text-xs font-bold text-cyan-400 uppercase tracking-wider mb-1">{lesson.category}</div>
                                    <h3 className="text-lg font-semibold text-white">{lesson.title}</h3>
                                </div>
                                <i className={`fas fa-chevron-down text-slate-400 transition-transform ${expandedId === lesson.id ? 'rotate-180' : ''}`}></i>
                            </div>
                            {expandedId === lesson.id && (
                                <div className="px-5 pb-5 pt-0 animate-up">
    ...
    const App = () => {
        // Default to 'workouts' (Page 1) as requested
        const [activeTab, setActiveTab] = useState('workouts');

        return (
            <div className="max-w-md mx-auto bg-slate-900 min-h-screen shadow-2xl relative border-x border-slate-800">
                <Header />
                ...
                <nav className="fixed bottom-0 w-full max-w-md bg-slate-900/95 backdrop-blur border-t border-slate-800 flex justify-between items-center z-50 pb-safe px-2">
                    <NavButton 
                        icon="fa-dumbbell" 
                        label="Workouts" 
